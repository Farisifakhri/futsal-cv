...
def run_pipeline(model_type="yolov8n.pt", imgsz=640, epochs=50):
    """
    ...
    Step 3: Modeling & Training YOLO (Default: YOLOv8n Nano Model)
    """
    ...
    # 4. Modeling menggunakan YOLO (Default: YOLOv8n.pt)
    ...
    stable_path_v8n = os.path.join('models', 'best_futsal_v8n.pt')
    ...
    if trained_best and os.path.exists(trained_best):
        shutil.copy(trained_best, stable_path_v8n)
        shutil.copy(trained_best, stable_path_default)
        print(f"Model terbaik ({model_name_clean}) berhasil disalin ke: {os.path.abspath(stable_path_v8n)}")
    else:
